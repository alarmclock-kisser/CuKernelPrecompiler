...
namespace CuKernelPrecompiler
{
	public class CudaHandling
	{
		...
		public void RunKernel(long firstPointer, float param1)
		{
			if (Ctx == null)
			{
				Log("No CUDA context available", "", 1);
			...
			// Get input variables
			List<CudaDeviceVariable<float2>> inputComplexVars = GetComplexVars(firstPointer, true);
			Dictionary<CUdeviceptr, long> pointers = new();

			// Speicher reservieren für prevPhase
			List<CudaDeviceVariable<float2>> prevPhaseVars = new();
			for (int i = 0; i < inputComplexVars.Count; i++)
			{
				CudaDeviceVariable<float2> prevPhase = new CudaDeviceVariable<float2>(inputComplexVars[i].Size);
				prevPhase.Memset(0); // Initialisieren mit 0
				prevPhaseVars.Add(prevPhase);
			}
			...
			for (int i = 0; i < pointers.Count; i++)
			{
				Kernel.Run(pointers.Values.ElementAt(i),
						   pointers.Keys.ElementAt(i),
						   param1,
						   prevPhaseVars[i].DevicePointer);
			}

			// Log completion
			sw.Stop();
			Log($"Kernel execution finished in {sw.ElapsedMilliseconds} ms", "", 2);
		}
	}
}
